...
private void btnReportActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnReportActionPerformed
    ...
    private void reportSummaryByFlightType() {
        ...
        int trainingWeight = 0;
        int totalWeight = 0;
        int totalFlights = 0;
        for(Flight f : allFlights){
            if (f.getFlightType().equals(Common.CARGO)) cargoWeight += f.calculateWeight();
            if (f.getFlightType().equals(Common.PASSENGER)) passengerWeight += f.calculateWeight();
            if (f.getFlightType().equals(Common.TRAINING)) trainingWeight += f.calculateWeight();
            totalWeight += f.calculateWeight();
            totalFlights++;
        }
        
        txtOutput.setText("Summary Report By FlightType\n\n");
        String output = "";
        output += String.format("%4d Cargo Flights\tWeight = %,d\n",cargo.length, cargoWeight);
        output += String.format("%4d Passenger Flights\tWeight = %,d\n",passenger.length, passengerWeight);
        output += String.format("%4d Training Flights\tWeight = %,d\n",training.length, trainingWeight);
        output += "----------------------------------------------------------------------\n";
        output += String.format("%4d Flights\tweight = %,d",totalFlights ,totalWeight);
    ...
    private void reportSummaryByDestination() {
        ...
        for(Location l : locations){
            int flights = 0;
            int weight = 0;
            Flight[] list = schedule.getFlightsByDestination(l.getLocationCode());
            flights = list.length;
            for(Flight f : list) weight += f.calculateWeight();
            totalFlights += flights;
            totalWeight =+ weight;
            if (flights > 0)
            txtOutput.append(String.format("%4d Flights to %s\tWeight = %,d\n",flights,l.getLocationCode(), weight));            
        }
        txtOutput.append("----------------------------------------------------------------------\n");
        txtOutput.append(String.format("%4d Flights\t\tWeight = %,d\n",totalFlights,totalWeight));
    ...
    private boolean createFlights(String fileName) {
        ...
        CSV_Parser p = new CSV_Parser();
        try {
            if (!p.open(fileName)) return false;
            while (p.getNextLine()) {
                String line = p.getLine();
                String[] tokens = line.split(",");
                
                if (tokens[0].equals(Common.CARGO)){
                    Flight f = new CargoFlight(tokens[1], tokens[2], 
                            Integer.parseInt(tokens[3]), findLocation(tokens[4]),
                            Integer.parseInt(tokens[5]), Integer.parseInt(tokens[6]));                    
                    result.add(f);
                }
                
                if (tokens[0].equals(Common.PASSENGER)){
                    Flight f = new PassengerFlight(tokens[1], tokens[2], 
                            Integer.parseInt(tokens[3]), findLocation(tokens[4]),
                            Integer.parseInt(tokens[5]), Integer.parseInt(tokens[6]));
                    result.add(f);
                }
                if (tokens[0].equals(Common.TRAINING)){
                    Flight f = new TrainingFlight(tokens[1], tokens[2], 
                            Integer.parseInt(tokens[3]), findLocation(tokens[4]),
            ...
            this.allFlights = result.toArray(Flight[]::new);
            return true;
        } finally {
            p.close();
        }
    ...
    private void runPolicyCheckReport() {
        ...
        String [] results = PolicyChecker.runPolicyChecks(allFlights);
        for (int i = 0;i < results.length; i++){
            if(!results[i].contains(Common.NO_ERROR)){
                txtOutput.append(allFlights[i].toArchiveFormat() + "\n");
